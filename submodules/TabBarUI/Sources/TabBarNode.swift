...
class TabBarNode: ASDisplayNode, ASGestureRecognizerDelegate {
    ...
    init(theme: TabBarControllerTheme, itemSelected: @escaping (Int, Bool, [ASDisplayNode]) -> Void, contextAction: @escaping (Int, ContextExtractedContentContainingNode, ContextGesture) -> Void, swipeAction: @escaping (Int, TabBarItemSwipeDirection) -> Void) {
        ...
        self.badgeImage = theme.squareStyle ? generateStretchableFilledRectImage(size: CGSize(width: 18, height: 18), cornerRadius: 4, color: theme.tabBarBadgeBackgroundColor, backgroundColor: nil)! : generateStretchableFilledCircleImage(diameter: 18.0, color: theme.tabBarBadgeBackgroundColor, strokeColor: theme.tabBarBadgeStrokeColor, strokeWidth: 1.0, backgroundColor: nil)!
    ...
    private func reloadTabBarItems() {
        ...
        for i in 0 ..< self.tabBarItems.count {
            ...
            if let selectedIndex = self.selectedIndex, selectedIndex == i {
                ...
                ContainedViewLayoutTransition.immediate.updateTransformScale(node: node.ringImageNode, scale: 1.0)
            } else {
    ...
    func updateLayout(size: CGSize, leftInset: CGFloat, rightInset: CGFloat, additionalSideInsets: UIEdgeInsets, bottomInset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if self.tabBarNodeContainers.count != 0 {
            ...
            let callsTabBarNodeContainer = tabBarNodeContainers.first(where: { $0.item.tag == 101 })
            
            if additionalSideInsets.right > 0.0 {
                width -= additionalSideInsets.right
                
                if let callsTabBarNodeContainer = callsTabBarNodeContainer {
                    if let callsTabIndex = tabBarNodeContainers.firstIndex(where: { $0 === callsTabBarNodeContainer }) {
                        tabBarNodeContainers.remove(at: callsTabIndex)
                    }
            ...
            for i in 0 ..< tabBarNodeContainers.count {
                ...
                if !self.showTabTitles && !horizontal {
                ...
                if horizontal {
                    ...
                    ringImageFrame = CGRect(origin: CGPoint(x: floorToScreenPixels((nodeSize.width - 29.0) / 2.0), y: self.showTabTitles ? 1.0 : 8.0), size: CGSize(width: 29.0, height: 29.0))
